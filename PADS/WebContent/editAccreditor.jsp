...
<c:set var="acc" scope="page" value="${accreditor}"/>
    <head>
	<!-- IMPORTS -->
...
$(document).ready(function() {
	...
	<c:forEach items="${accreditor.getWorks()}" var="work">
		...
		if(to == ""){
			var add = "<li class='list-group-item' id='work" + globalWork + "'>"+inst+"<ul class='list-group'><li class='list-group-item'>Position: " + pos + "</li><li class='list-group-item'>From " + from + " to present</li></ul></li>";

		}else{
			var add = "<li class='list-group-item' id='work" + globalWork + "'>"+inst+"<ul class='list-group'><li class='list-group-item'>Position: " + pos + "</li><li class='list-group-item'>From " + from + " to " + to + "</li></ul></li>";
	...
	var lel = globalWork;
		...
		btnX.onclick = function(){
			$("#work"+lel).remove();
			affObject.works = affObject.works.filter(function(item){
				return item.id !== lel;
	...
	<c:forEach items="${accreditor.getDegrees()}" var="deg">
		...
		var add = "<li class='list-group-item' id='edu" + globalEdu + "'>"+univ+"<ul class='list-group'><li class='list-group-item'>Course: " + course + "</li></ul></li>";
		...
		btnX.onclick = function(){
			$("#edu"+lel).remove();
			affObject.works = affObject.edu.filter(function(item){
				return item.id !== lel;
...
function saveAccreditor(){
	...
	var datetrained = $('datepicker').val();
	var city = $('city').val();
	alert("-"+primaryArea+"-")
	if(firstName == ""){
	...
	}else if(primaryArea == ""||primaryArea == "undefined"){
...
function addEducation(){
	...
	if(course == ""){
		...
		btnX.onclick = function(){
			$("#edu"+lel).remove();
			affObject.edu = affObject.edu.filter(function(item){
				return item.id !== lel;
			});
	   	};	      
        ...
        <div class="main-wrapper">
			...
			<section class="section" id="section" style="height:450px;"> 		
      				...
      				<div class="col-xs-5">
    					...
    					<input type="text" class="form-control underlined" id="firstName" name="firstName" value="${acc.getFirstName()}">
    				</div>
    				<div class="col-xs-1">
						<label>M.I.:</label>
    					<input type="text" class="form-control underlined" id="middleName" name="middleName" value="${acc.getMiddleName()}">
    				</div>
      				<div class="col-xs-4">
    					<label>Last Name:</label>
    					<input type="text" class="form-control underlined" id="lastName" name="lastName" value="${acc.getLastName()}">
    				</div>
    				</div>
    				...
    				<div class="col-xs-7">
						<label>Email:</label>
    					<input type="text" class="form-control underlined" id="email" name="email" value="${acc.getEmail()}">
    				</div>
    				<div class="col-xs-5">
    					<label>Contact Number:</label>
    					<input type="text" class="form-control underlined" id="contactNum" name="contact" value="${acc.getContact()}">
    				</div>
    				</div>
    				<div class="form-group row">
    				<div class="col-xs-7">
						<label>Address:</label>
  						<input type="text" class="form-control underlined" id="address" name="address" value="${acc.getAddress()}">
					</div>
					<div class="col-xs-5">
						<label>City:</label>
  						<input type="text" class="form-control underlined" id="city" name="city" value="${acc.getCity()}">
					</div>
  					</div>
					...
					<div class="col-xs-7">
						<label><b>Country:</b></label>
  						<input type="text" class="form-control underlined" id="country" name="country" value="${acc.getCountry()}">
					</div>
    				<div class="col-xs-5">
					...
					<div class="col-xs-7">
						<label><b>Venue Trained:</b></label>
  						<input type="text" class="form-control underlined" id="venueTrained" name="venue_trained" value="${acc.getVenue_trained()}">
					</div>
    				<div class="col-xs-5">
						<label><b>Date Trained:</b></label>
  						<input type="text" class="form-control underlined" id="datepicker" name="date_trained" value="${acc.getDate_trained()}">
					</div>
