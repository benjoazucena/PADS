...
// 				 if(value.programs[i].surveyType == "Resurvey"){ctype = "resurveyConfirm";};
		...
		for(var i = 0; i < value.programs.length; i++){ 
			var link = getLink(value.programs[i].surveyType,i, ID);
			 ...
			 add += ("<div class='col-md-12' style='position:relative; left:-15px;''><label><h5><p id='program"+value.programs[i].PSID+"'>" + value.programs[i].programName + "</p> - " + value.programs[i].surveyType + "</h5></label>"); 
			    	...
			    	add += (" <tbody> ");
			    	 ...
			    	 for(var j = 0; j < value.programs[i].areas.length; j++){
			    		...
			    		add += ("<td><a id='accLink"+value.programs[i].areas[j].areaID +"' href='ViewAccreditor?accreditorID=" + value.programs[i].areas[j].accreditorID + "' data-toggle='tooltip' title='This will take you to the accreditor page.'>"+ value.programs[i].areas[j].accreditor + "</a></td><td id='area"+value.programs[i].areas[j].areaID +"'>" + value.programs[i].areas[j].area + "</td>");
			   				...
			   				add += ("<td><label><input type='checkbox' "+ checked +" onclick='confirmAccreditor(" + value.programs[i].PSID + "," + value.programs[i].areas[j].areaID + "," + value.programs[i].areas[j].accreditorID + ")' class='checkbox rounded' value='Confirm Attendance' id='checkbox_confirm"+value.programs[i].areas[j].accreditorID+"'><span>Confirm</span></label> <button class='btn btn-link btn-sm' onclick='changeAcc(" + value.programs[i].areas[j].areaID + ","+value.id+","+value.programs[i].PSID+")' id='changedbutton"+value.programs[i].areas[j].accreditorID+"'><i class='fa fa-pencil'></i>Changed</button></td>");
...
i=+1;
	alert(type);
	if(type=="Formal"){return "'formalConfirm("+i+","+ID+")'"};
	if(type=="Consultancy"){return "'consultancyConfirm("+i+","+ID+")'"};
	if(type=="Preliminary"){return "'preliminaryConfirm("+i+","+ID+")'"};
...
function consultancyConfirm(PSID, surveyID) {
	var add = "";
	alert(PSID+"hhhhhh");
    ...
    add += ("<form method='post' action='SurveyProgramDecision'>");
   	 add += ("<div id='modalBody' class='modal-body'>	");
   		 add += ("<div class='form-group' style='width:100%;'> <div> 	");
   			 ...
   			 add += ("<label> <input type='hidden' name='PSID' value='"+PSID+"'><input type='hidden' name='surveyID' value='"+surveyID+"'> <input type='hidden' name='type'value='Consultancy'> <input name='opt'class='radio' type='radio'>   <span>Accreditation not granted</span> </label> <br>");
...
function changeAcc(areaID,  programID, systemID){
	...
	//ONCLICK OF ASSIGNING ACCREDITOR
	 $('#smarttable tbody').on('click', 'tr', function () {
			var chosenAccreditor = table.row(this).data();
       		...
       		$.ajax({ //CALLING ACCREDITORS WITH EXTRA CHECKING FOR AFFILIATION CONFLICTS
      		  url: "ChangeAccreditor?PSID=" + programID +"&AreaID=" + areaID+"&AccreditorID=" + chosenAccreditor[0],
...
// 	<div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span> <span class="sr-only">close</span></button><hh4 id="modalTitle" class="modal-title">Computer Science - Consultancy Survey (Survey Details)</h4></div>
    ...
    add += ("<form method='post' action='SurveyProgramDecision'>");
   	 add += ("<div id='modalBody' class='modal-body'>	");
   		 add += ("<div class='form-group' style='width:100%;'> <div> 	");
   		 add += "wew";
   			 add += ("<label class='control-label' style='width:100%;'> <select><option>Team</option></select><input name='opt' class='radio ' type='radio'>	<span>Eligible for formal survey in </span> <input type='text' name='duration' class='form-control underlined' style='width:15%;'  placeholder='e.g. 5'><span> months </span><br> <hr></label>");
   			 add += ("<label> <input type='hidden' name='PSID' value='"+PSID+"'><input type='hidden' name='surveyID' value='"+surveyID+"'> <input type='hidden' name='type'value='Consultancy'> <input name='opt'class='radio' type='radio'>   <span>Accreditation not granted</span> </label> <br>");
  		 add += ("<div class='form-group'> <label class='control-label'>Reason(s) for denial</label> <br> <textarea name='remarks' rows='3' value = '' class='form-control' style='width:100%;'></textarea> </div>");
   	 add += ("</div> </div></div>");
...
function formalConfirm(PSID, surveyID) {
	var add = "";
	alert(PSID+"hhhhhh");
...
function preliminaryConfirm(PSID, surveyID) {
	var add = "";
	alert(PSID+"hhhhhh");
...
// 	<div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span> <span class="sr-only">close</span></button><hh4 id="modalTitle" class="modal-title">Computer Science - Consultancy Survey (Survey Details)</h4></div>
    ...
    add += ("<form method='post' action='SurveyProgramDecision'>");
   	 add += ("<div id='modalBody' class='modal-body'>	");
   		 add += ("<div class='form-group' style='width:100%;'> <div> 	");
   			 add += ("<label class='control-label' style='width:100%;'> <input name='opt' class='radio ' type='radio' value=''><span>Eligible for formal survey in </span> <input type='text' name='duration' class='form-control underlined' style='width:15%;'  placeholder='e.g. 5'><span> months </span><br> <hr></label>");
   			 add += ("<label> <input type='hidden' name='PSID' value='"+PSID+"'><input type='hidden' name='surveyID' value='"+surveyID+"'> <input type='hidden' name='type'value='Consultancy'> <input name='opt'class='radio' type='radio'>   <span>Accreditation not granted</span> </label> <br>");
  		 add += ("<div class='form-group'> <label class='control-label'>Reason(s) for denial</label> <br> <textarea name='remarks' rows='3' value = '' class='form-control' style='width:100%;'></textarea> </div>");
   	 add += ("</div> </div></div>");
...
function resurveyConfirm(PSID, surveyID) {
	var add = "";
	alert(PSID+"hhhhhh");
...
// 	<div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span> <span class="sr-only">close</span></button><hh4 id="modalTitle" class="modal-title">Computer Science - Consultancy Survey (Survey Details)</h4></div>
    ...
    add += ("<form method='post' action='SurveyProgramDecision'>");
   	 add += ("<div id='modalBody' class='modal-body'>	");
   		 add += ("<div class='form-group' style='width:100%;'> <div> 	");
   			 add += ("<label class='control-label' style='width:100%;'> <input name='opt' class='radio ' type='radio' value='Accreditation grant for 3 years'>	<span>Accreditation grant for 3 years </span> <br> <hr></label>");
   			 add += ("<label> <input type='hidden' name='PSID' value='"+PSID+"'><input type='hidden' name='surveyID' value='"+surveyID+"'> <input type='hidden' name='type'value='Consultancy'> <input name='opt'class='radio' type='radio' value='Accreditation Denied'   <span>Accreditation not granted</span> </label> <br>");
  		 add += ("<div class='form-group'> <label class='control-label'>Reason(s) for denial</label> <br> <textarea name='remarks' rows='3' value = '' class='form-control' style='width:100%;'></textarea> </div>");
   	 add += ("</div> </div></div>");
